...
# 修改文件存储路径为统一的 /tmp/downloads
DOWNLOAD_DIR = Path("/tmp/downloads")  # Vercel 只允许写入 /tmp 目录
...
async def download_video(url: str = Form(...)):
    try:
        output_path = Path("/tmp/downloads")
        # 执行下载逻辑
        return {"status": "success", "message": "开始下载视频"}
    except Exception as e:
        return JSONResponse(
            status_code=500,
